...
public class ItemBoatRope extends Item {
	...
	public void onUseForBoat(EntityMinecart entity,EntityPlayer entityplayer,ItemStack itemstack,World world){
		if(!world.isRemote){
			...
			if(nbt==null){
				...
				if(!nbt.hasKey("UUIDMost")){
					...
					if(world instanceof WorldServer){
						ChatComponentText chat=new ChatComponentText("Binding...");
						entityplayer.addChatComponentMessage(chat);
					}
				}else {
					if(entity.getUniqueID().getMostSignificantBits()==nbt.getLong("UUIDMost")&&
						...
						ChatComponentText chat=new ChatComponentText("Canceled.");
						entityplayer.addChatComponentMessage(chat);
						...
						if(entity1 instanceof EntityBoatWithBelayingPin){
							...
							ChatMessageComponent chat=new ChatMessageComponent();
							chat.addText("Succeeded to bind.");
							((WorldServer)world).getMinecraftServer().getConfigurationManager().sendChatMsg(chat);
						}
