...
public class EntityStone extends Entity
{
	...
	public int timeToDisappearMax=1;
	...
	public void onUpdate()
	{
		...
		for(int i=1;i<=j;i++){
								...
								if(tAABB!=null){
									if(this.boundingBox.intersectsWith(AxisAlignedBB.getBoundingBox(tAABB.minX+x,tAABB.minY+y,tAABB.minZ+z,
										...
										if(target.getBlockHardness(worldObj, x, y, z)==-1){
											...
											hardness=1.5*(target.getBlockHardness(worldObj, x, y, z)+(2.0-target.getBlockHardness(worldObj, x, y, z))/2)/this.destruction;
										}
			...
			this.posY = (this.boundingBox.minY + this.boundingBox.maxY)/2.0D-this.eyeHeight;
	...
	public void makeTicket(){
		if(this.ticket==null){
			ForgeChunkManager.Ticket chunkTicket=ForgeChunkManager.requestTicket(MonsterRegularArmyCore.instance, this.worldObj, ForgeChunkManager.Type.ENTITY);
			if (chunkTicket != null)
			{
	...
	public void onCollidedWithEntity(Entity entity){
		...
		if(relativeSpeedSquare>0.25){
			...
			double damage=this.damage*(MathHelper.sqrt_double(relativeSpeedSquare)-0.5);
