...
public class ModelCatapult extends ModelBase
{
	...
	public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5,EntityCatapult entity)
	{
		...
		if(this.onGround<0.01f){
			arms.rotateAngleX=defaultAngleArm;
		}else if(this.onGround<swingStartf){
			arms.rotateAngleX=defaultAngleArm+(minAngleArm-defaultAngleArm)*MathHelper.sin(this.onGround/swingStartf*(float)Math.PI*0.5f);
		}else if(this.onGround<swingEndf){
			
			arms.rotateAngleX=minAngleArm+(maxAngleArm-minAngleArm)*(this.onGround-swingStartf)/(swingEndf-swingStartf);
		}else{
			arms.rotateAngleX=maxAngleArm+(defaultAngleArm-maxAngleArm)*(this.onGround-swingEndf)/(1-swingEndf);
		}
	}
}
