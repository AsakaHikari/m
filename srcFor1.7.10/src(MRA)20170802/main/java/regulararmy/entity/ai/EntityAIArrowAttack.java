    ...
    public void updateTask()
    {
        ...
        if(this.entityHost.getANavigator().noPath()){
        	if(squareDistance > (double)this.maxAttackRange2x ){
        		...
        		(entityHost.posX+this.getSplitRandom(5, 10), entityHost.posY+entityHost.worldObj.rand.nextInt(5)-2, entityHost.posZ+this.getSplitRandom(5, 10),
        ...
        if(this.doTurn){
            ...
            float d3 = (float)MathHelper.sqrt_double(d0 * d0 + d2 * d2);
        ...
        if (!isLaggy() && --this.rangedAttackTime == 0)
        {
