...
public class ContainerCannon extends Container {
	...
	public ContainerCannon(InventoryPlayer invPlayer,EntityCannon entity){
		...
		for(int i=0;i<(entityCannon.ammoItems.length-1)/4+1;i++){
			for(int j=0;j<4;j++){
				if(i*4+j>=entityCannon.ammoItems.length-1)break;
		...
		slotsSoFar++;
		for(int i=0;i<(entityCannon.gunPowderItems.length-1)/4+1;i++){
			for(int j=0;j<4;j++){
				if(i*4+j>=entityCannon.gunPowderItems.length-1)break;
	...
	public int getGunPowderSlotFrom(){
		return this.getAmmoSlotFrom()+this.entityCannon.ammoItems.length;
	}

	public int getFuelSlot(){
		return this.getGunPowderSlotFrom()+this.entityCannon.gunPowderItems.length;
	}
