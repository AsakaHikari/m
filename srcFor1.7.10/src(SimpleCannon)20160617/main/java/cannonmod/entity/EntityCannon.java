...
public class EntityCannon extends EntityLivingBase implements IInventory{
	public ItemStack[] ammoItems;
	public ItemStack[] gunPowderItems;
	...
	public EntityCannon(World par1World) {
		...
		this.ammoItems=new ItemStack[9];
		this.gunPowderItems=new ItemStack [9];
	...
	public void onEntityUpdate(){
		...
		if (this.riddenByEntity != null && this.riddenByEntity instanceof EntityLivingBase)
		{
			...
			if(this.engine>0 && 
				...
				if(this.fuelLeft<0){
					ItemStack stack=this.getStackInSlot(this.ammoItems.length+this.gunPowderItems.length);
					if(stack!=null && TileEntityFurnace.isItemFuel(stack)){
						...
						if(stack.stackSize==0){
							this.setInventorySlotContents(this.ammoItems.length+this.gunPowderItems.length, stack.getItem().getContainerItem(stack));
						}
					}
				}
		...
		if(!p_70665_1_.isFireDamage()){ 
			...
			if(source!=null){
				...
				if (source instanceof EntityLivingBase){
					...
					if(tool!=null && ForgeHooks.isToolEffective(tool, Blocks.iron_block, 0)){
	...
	public void updateInventory(){
		for(int i=0;i<this.ammoItems.length-1;i++){
			if(this.ammoItems[i]==null && this.ammoItems[i+1]!=null){
				this.setInventorySlotContents(i, this.ammoItems[i+1]);
				this.setSlotNull(i+1);
			}
		}
		for(int i=0;i<this.gunPowderItems.length-1;i++){
			if(this.gunPowderItems[i]==null && this.gunPowderItems[i+1]!=null){
				this.setInventorySlotContents(i+this.ammoItems.length, this.gunPowderItems[i+1]);
				this.setSlotNull(i+1+this.ammoItems.length);
			}
		}
	}
	@Override
	...
	public void setSlotNull(int num) {
		...
		if(num<this.ammoItems.length){
			this.ammoItems[num] =null;
		}else if(num<this.ammoItems.length+this.gunPowderItems.length){
			this.gunPowderItems[num-this.ammoItems.length]=null;
		}else{
			this.fuelItem=null;
		}
	}
	...
	public int getSizeInventory() {
		return this.ammoItems.length+this.gunPowderItems.length+1;
	}

	@Override
	public ItemStack getStackInSlot(int num) {
		if(num>this.getSizeInventory())return null;
		return num<this.ammoItems.length ? this.ammoItems[num] : num<(this.ammoItems.length+this.gunPowderItems.length)?this.gunPowderItems[num-this.ammoItems.length]:this.fuelItem;
	}
	...
	public ItemStack getStackInSlotOnClosing(int p_70304_1_) {
		ItemStack stackInSlot=this.getStackInSlot(p_70304_1_);
		if(stackInSlot!=null){
			this.setSlotNull(p_70304_1_);
			return stackInSlot;
		}else{
			return null;
		}
	...
	public void setInventorySlotContents(int num, ItemStack stack) {
		if(num>this.getSizeInventory())return;
		if(num<this.ammoItems.length){
			this.ammoItems[num] =stack;
		}else if(num<this.ammoItems.length+this.gunPowderItems.length){
			this.gunPowderItems[num-this.ammoItems.length]=stack;
		}else{
			this.fuelItem=stack;
		}
	}
	...
	public void openFire(){
		...
		if(this.getStackInSlot(this.ammoItems.length)==null){
		...
		if(!(this.worldObj.getBlock(bx, by, bz) instanceof BlockAir)){
			Entity entity=this.riddenByEntity;
			if(entity instanceof EntityPlayer){
		 ...
		 */
		//dispense
		Block block=this.worldObj.getBlock(fx, fy, fz);

		this.worldObj.setBlock(bx, by, bz, Blocks.dispenser);
		this.worldObj.setBlockMetadataWithNotify(bx, by, bz, meta, 3);
		TileEntity t=this.worldObj.getTileEntity(bx, by, bz);
		if(t instanceof TileEntityDispenser){
			TileEntityDispenser te=(TileEntityDispenser) this.worldObj.getTileEntity(bx, by, bz);
			...
			for(int i=0;i<((this.calibre/5)*(this.calibre/5)*2);i++){
				...
				Blocks.dispenser.updateTick(this.worldObj, bx, by, bz, this.worldObj.rand);
			...
			Block block1=this.worldObj.getBlock(fx, fy, fz);
			if(block != block1){

				EntityFallingBlockEx entity=new EntityFallingBlockEx(this.worldObj,fx+0.5, fy+0.5, fz+0.5,block1,
						this.worldObj.getBlockMetadata(fx, fy, fz));
				NBTTagCompound nbt=new NBTTagCompound();
				TileEntity tileentity=this.worldObj.getTileEntity(fx, fy, fz);
				if(tileentity!=null){
				...
				(entity,new S0EPacketSpawnObject(entity, 70, Block.getIdFromBlock(entity.func_145805_f()) | entity.blockMetadata<< 16));
				this.worldObj.setBlock(fx, fy, fz,Blocks.air);
				...
				if(!((i<entityListCopy.size() && e==entityListCopy.get(i)) || entityListCopy.contains(e))){
					if(e instanceof EntityItem ){
						...
						if(entityitem.getEntityItem().getItem() instanceof ItemBlock){
							...
							if(item.field_150939_a.canPlaceBlockAt(this.worldObj, fx, fy, fz) &&
									this.worldObj.setBlock(fx, fy, fz, item.field_150939_a, item.getMetadata(stack.getItemDamage()), 2)){


								EntityFallingBlockEx entity=new EntityFallingBlockEx(this.worldObj,fx+0.5, fy+0.5, fz+0.5,item.field_150939_a,
										this.worldObj.getBlockMetadata(fx, fy, fz));
								NBTTagCompound nbt=new NBTTagCompound();
								TileEntity tileentity=this.worldObj.getTileEntity(fx, fy, fz);
								if(tileentity!=null){
			...
			WorldServer worldserver=(WorldServer)this.worldObj;
			 if (this.calibre >= 15)
		        {
				 	worldserver.func_147487_a("hugeexplosion", x,y,z,1, 1.0D, 0.0D, 0.0D,0.15d);
		        }
		        else
		        {
		        	worldserver.func_147487_a("largeexplode", x,y,z,1, 1.0D, 0.0D, 0.0D,0.07d);
		        }
			...
			double speed=this.bulletSpeed*MathHelper.sqrt_float(this.getStackInSlot(this.ammoItems.length).stackSize)/MathHelper.sqrt_double(volumeSum);
			
			this.setSlotNull(this.ammoItems.length);
	...
	public void readFromNBT(NBTTagCompound tagCompound) {
		super.readFromNBT(tagCompound);
		{
			...
			for (int i = 0; i < tagList.tagCount(); i++) {
				...
				if (slot >= 0 && slot < this.ammoItems.length) {
					this.ammoItems[slot] = ItemStack.loadItemStackFromNBT(tag);
				}
			}
		}
		{
			...
			for (int i = 0; i < tagList.tagCount(); i++) {
				...
				byte slot = tag.getByte("Slot");
				if (slot >= 0 && slot < this.ammoItems.length) {
					this.gunPowderItems[slot] = ItemStack.loadItemStackFromNBT(tag);
				}
			}
		}
		this.fuelItem=ItemStack.loadItemStackFromNBT(tagCompound.getCompoundTag("Fuel"));
	...
	public void writeToNBT(NBTTagCompound tagCompound) {
		super.writeToNBT(tagCompound);
		{
			...
			for (int i = 0; i < this.ammoItems.length; i++) {
				ItemStack stack = this.ammoItems[i];
				if (stack != null) {
		...
		{
			NBTTagList itemList = new NBTTagList();
			for (int i = 0; i < this.gunPowderItems.length; i++) {
				ItemStack stack = this.gunPowderItems[i];
				if (stack != null) {
					NBTTagCompound tag = new NBTTagCompound();
					tag.setByte("Slot", (byte) i);
					stack.writeToNBT(tag);
