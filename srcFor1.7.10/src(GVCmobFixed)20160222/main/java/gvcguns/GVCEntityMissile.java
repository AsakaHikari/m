...
public class GVCEntityMissile extends EntityThrowable
{
	...
	public GVCEntityMissile(World par1World)
    {
        super(par1World);
        this.fuse = 200;
    }

    public GVCEntityMissile(World par1World, EntityLivingBase par2EntityLivingBase, int damege, float bspeed, float bure, float explv, boolean fb, boolean bb)
    {
    ...
    public GVCEntityMissile(World par1World, double par2, double par4, double par6)
    {
 ...
 // ホーミングミサイルの向いている方向
    ...
    public void onUpdate()
    {
        ...
        List llist = this.worldObj.getEntitiesWithinAABBExcludingEntity(entity, this.boundingBox.addCoord(this.motionX, this.motionY, this.motionZ).expand(20.0D, 20.0D, 20.0D));
        double d0 = 8.0D;
        EntityLivingBase entitylivingbase = this.getThrower();
        if(llist!=null){
            for (int lj = 0; lj < llist.size(); lj++) {
            	...
            	if (entity1.canBeCollidedWith() && (entity1 != entitylivingbase || this.ticksInAir >= 5))
                {
            		...
            		d0 = 8.0D;
                        ...
                        {
                            d5 *= d5;
                            this.motionX += d1 / d4 * d5 * 2.0D;
                            this.motionY += d2 / d4 * d5 * 2.0D;
                            this.motionZ += d3 / d4 * d5 * 2.0D;
                        }
                    }
                }
            }
