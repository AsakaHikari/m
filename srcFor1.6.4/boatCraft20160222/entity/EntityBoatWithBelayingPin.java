package boatCraft.entity;

import java.util.*;

import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;
import boatCraft.core.BoatCraftCore;
...
public class EntityBoatWithBelayingPin extends Entity {
 public UUID boundByEntity,bindingEntity;//The front boat is binding,and the back boat is bound.
 public static Map UUIDMap=new HashMap<UUID,Integer>();
	   private boolean field_70279_a;
	    ...
	    public EntityBoatWithBelayingPin(World par1World)
	    {
	        super(par1World);
	    ...
	    public EntityBoatWithBelayingPin(World par1World, double par2, double par4, double par6)
	    {
	        this(par1World);
	...
	public boolean interactFirst(EntityPlayer par1EntityPlayer){
        ...
        if (itemstack != null && itemstack.itemID == BoatCraftCore.itemBoatRope.itemID)
        {
        	if(!worldObj.isRemote)
        	BoatCraftCore.itemBoatRope.onUseForBoat(this,par1EntityPlayer,par1EntityPlayer.inventory.getCurrentItem(),this.worldObj);
	...
	public void addToUUIDMap(){
		UUIDMap.put(this.getUniqueID(), this.entityId);
		
	}
	
	@Override
	public void onUpdate(){
		if(this.boundByEntity!=null){
			Entity entity=worldObj.getEntityByID((Integer) UUIDMap.get(boundByEntity));
			if(entity==null||entity.isDead){
				this.boundByEntity=null;
			}else{
				AxisAlignedBB beAABB=this.getBoundingBox();
				AxisAlignedBB eAABB=entity.getBoundingBox();
			
				this.motionX=entity.posX+((beAABB.maxX-beAABB.minX)/2+(eAABB.maxX-eAABB.minX)/2+0.8)*Math.cos(entity.rotationYaw*Math.PI/180.0)-this.posX;
				this.motionZ=entity.posZ+((beAABB.maxZ-beAABB.minZ)/2+(eAABB.maxZ-eAABB.minZ)/2+0.8)*Math.sin(entity.rotationYaw*Math.PI/180.0)-this.posZ;
			}
		}
	      super.onUpdate();
	        ...
	        //System.out.println("boatX="+boatX+",boatY="+boatY+",boatZ="+boatZ);
	        //System.out.println(this.speedMultiplier);
        
		/*
		double x,y,z,x_,y_,z_;
		x=this.posX+(this.getBoundingBox().maxX-this.getBoundingBox().minX)*Math.cos(this.rotationYaw/180.0f*Math.PI);
		y=this.posY+(this.getBoundingBox().maxY-this.getBoundingBox().minY);
		z=this.posZ+(this.getBoundingBox().maxZ-this.getBoundingBox().minZ)*Math.sin(this.rotationYaw/180.0f*Math.PI);
		EntityBoatWithBelayingPin boundByEntity_=(EntityBoatWithBelayingPin)(UUIDMap.get(this.boundByEntity));
		x_=boundByEntity_.posX-(boundByEntity_.getBoundingBox().maxX-boundByEntity_.getBoundingBox().minX)*Math.cos(boundByEntity_.rotationYaw/180.0f*Math.PI);
		y_=boundByEntity_.posY-(boundByEntity_.getBoundingBox().maxY-boundByEntity_.getBoundingBox().minY);
		z_=boundByEntity_.posZ-(boundByEntity_.getBoundingBox().maxZ-boundByEntity_.getBoundingBox().minZ)*Math.sin(boundByEntity_.rotationYaw/180.0f*Math.PI);
		if((x-x_)*(x-x_)+(y-y_)*(y-y_)+(z-z_)*(z-z_)>1.0){
