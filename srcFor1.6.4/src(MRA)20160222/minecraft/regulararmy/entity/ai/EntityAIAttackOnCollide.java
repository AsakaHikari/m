...
public class EntityAIAttackOnCollide extends EntityRegularAIBase implements IPathFindRequester
{
    ...
    AStarPathEntity entityPathEntity;
    Class classTarget;
    private int field_75445_i;

    private int failedPathFindingPenalty;

    public EntityAIAttackOnCollide(EntityRegularArmy par1EntityCreature, Class par2Class, double par3, boolean par5)
    {
    ...
    public boolean shouldExecute()
    {
        ...
        else
        {
            if (-- this.field_75445_i <= 0)
            {
                ...
                this.entityPathEntity = this.attacker.getANavigator().getPathToEntityLiving(entitylivingbase,MathHelper.sqrt_double((double)(this.attacker.width * 2.0F * this.attacker.width * 2.0F + entitylivingbase.width)-0.2),this);
    ...
    {
        EntityLivingBase entitylivingbase = this.attacker.getAttackTarget();
        return entitylivingbase == null ? false : (!entitylivingbase.isEntityAlive() ? false : (!this.longMemory ? !this.attacker.getANavigator().noPath() : this.attacker.func_110176_b(MathHelper.floor_double(entitylivingbase.posX), MathHelper.floor_double(entitylivingbase.posY), MathHelper.floor_double(entitylivingbase.posZ))));
    }
    ...
    public void updateTask()
    {
        ...
        if ((this.longMemory || this.attacker.getEntitySenses().canSee(entitylivingbase)) && --this.field_75445_i <= 0)
        {
            ...
            this.attacker.getANavigator().tryMoveToEntityLiving(entitylivingbase, this.speedTowardsTarget,2,this);
