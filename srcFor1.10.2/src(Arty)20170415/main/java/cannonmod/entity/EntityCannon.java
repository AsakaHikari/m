...
public class EntityCannon extends EntityLivingBase implements IInventory{
	...
	public static final DataParameter<Optional<ItemStack>>[] symbolPar=new DataParameter[symbolNum];
	...
	static{
		for(int i=0;i<symbolNum;i++){
			symbolPar[i]=EntityDataManager.<Optional<ItemStack>>createKey(EntityCannon.class,DataSerializers.OPTIONAL_ITEM_STACK);
	...
	public void entityInit(){
		...
		for(int i=0;i<symbolNum;i++){
			this.getDataManager().register(symbolPar[i],Optional.fromNullable((ItemStack)null));
	...
	public void updateDataWatcher(){
		...
		for(int i=0;i<symbolNum;i++){
			this.getDataManager().set(symbolPar[i],Optional.fromNullable(this.inventory[this.getSymbolSlotNum(i)]));
			if(this.arts[i]!=null){
	...
	public void setStats(){
		...
		double red=MathHelper.sqrt_double(this.armor/this.size*0.02);
	...
	public void onEntityUpdate(){
		...
		{
			List<Entity> list = this.worldObj.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox().expand(0.2, 0.2, 0.2), EntitySelectors.<Entity>getTeamCollisionPredicate(this));
		...
		List<Entity> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this,this.getEntityBoundingBox().expand(0.5, 0.5, 0.5));
		if (!list.isEmpty())
		{
			for (int j1 = 0; j1 < list.size(); ++j1)
			{
				...
				if(entity1 instanceof EntityItem && !entity1.isDead){
					...
					if(!entityitem.cannotPickup()){
						...
						if(entityitem.getEntityItem().getItem() == Items.GUNPOWDER){
							if(this.getStackInSlot(this.ammoSlot+this.gunpowderSlot-1)==null){
								this.setInventorySlotContents(this.ammoSlot+this.gunpowderSlot-1, entityitem.getEntityItem());
								this.onItemPickup(entityitem, entityitem.getEntityItem().stackSize);
								entityitem.setDead();
							}
						}else{
							if(this.getStackInSlot(this.ammoSlot-1)==null){
								this.setInventorySlotContents(this.ammoSlot-1, entityitem.getEntityItem());
								this.onItemPickup(entityitem, entityitem.getEntityItem().stackSize);
								entityitem.setDead();
							}
						}
								...
								if(MathHelper.abs(entitylivingbase.moveForward)<=0.001){
									...
									if(this.worldObj.isRemote){
										for(int i=0;i<this.engine;i++){
											this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_LARGE, this.posX+this.vehicleLength*MathHelper.sin(rotationRd)/2+(this.rand.nextGaussian())
...
/**a130*/
		...
		if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE)
		{
			...
			if(this.worldObj.isRemote){
				for(int i1=0;i1<this.size*2+1;i1++){
					this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK,xBase1+this.vehicleLength*(this.rand.nextDouble()-0.5)/4
					...
					this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK,xBase2+this.vehicleLength*(this.rand.nextDouble()-0.5)/4
	...
	protected void damageEntity(DamageSource dmgSource, float dmgValue)
	{
		if(!dmgSource.isFireDamage()){ 
			Entity source= dmgSource.getSourceOfDamage();
			...
			if(source!=null){
				...
				/**a120*/
				double prob=0.4f*MathHelper.sqrt_float(dmgValue) * volume*this.size/this.armor/2*(1+Math.sqrt(source.motionX*source.motionX+source.motionY*source.motionY+source.motionZ*source.motionZ));
				...
				/**a120*/
				double prob=0.4f*MathHelper.sqrt_float(dmgValue) * this.size/this.armor/2;
				//System.out.println("prob:"+prob+" armor:"+this.armor+" size:"+this.size);
				if(this.worldObj.rand.nextDouble()<prob){
					super.damageEntity(dmgSource, dmgValue);
					return;
	...
	{
		EntityTNTPrimed tnt=new EntityTNTPrimed(this.worldObj,this.posX,this.posY,this.posZ,this.getLastAttacker());
		tnt.setFuse(0);
	...
	public ItemStack getThisItemStack(){
		...
		nbt.setInteger("HPLeft",MathHelper.ceiling_float_int(this.getHealth()));
	...
	public void openFire(){
		...
		if(this.getStackInSlot(this.ammoSlot)==null){
			...
			if(entity instanceof EntityPlayer){
				((EntityPlayer)entity).addChatMessage(new TextComponentString("No Gunpowder!"));
			}
			return;
		}
		if(this.getStackInSlot(0)==null){
			Entity entity=riddenByEntity;
			if(entity instanceof EntityPlayer){
				((EntityPlayer)entity).addChatMessage(new TextComponentString("No Ammo!"));
			}
			return;
		}
		...
		if(!(this.worldObj.getBlockState(new BlockPos(bx, by, bz)).getBlock() instanceof BlockAir)){
			Entity entity=riddenByEntity;
			if(entity instanceof EntityPlayer){
				((EntityPlayer)entity).addChatMessage(new TextComponentString("A block is blocking fire!"));
			}
			return;
		}
		...
		if(t instanceof TileEntityDispenser){
			...
			if(firedEntityList.size()==0){
				Entity entity=riddenByEntity;
				if(entity instanceof EntityPlayer){
					((EntityPlayer)entity).addChatMessage(new TextComponentString("Something is blocking fire!"));
				}
				return;
			}
			this.reloadtime=0;
			...
			if(this.calibre<=15){
				this.sendFireSuccessCommand(x,y,z,mx,my,mz,this.getStackInSlot(this.ammoSlot).stackSize,(byte)0);
			}else{
				this.sendFireSuccessCommand(x,y,z,mx,my,mz,this.getStackInSlot(this.ammoSlot).stackSize,(byte)1);
			}
			...
			for(int i=0;i<firedEntityList.size();i++){
				...
				EntityTracer tracer=new EntityTracer(this.worldObj,entity,shooter);
				this.worldObj.spawnEntityInWorld(tracer);
	...
	public void readFromNBT(NBTTagCompound tagCompound) {
		...
		/**a130*/
		this.inventory[this.getFuelSlotNum()]=ItemStack.loadItemStackFromNBT(tagCompound.getCompoundTag("Fuel"));
		for(int i=0;i<symbolNum;i++){
			this.inventory[this.getSymbolSlotNum(i)]=ItemStack.loadItemStackFromNBT(tagCompound.getCompoundTag("Symbol"+i));
	...
	public ModelCannon getThisModel(RenderCannon renderer){
		if(this.model==null){
			...
			for(int i=0;i<symbolNum;i++){
				this.inventory[this.getSymbolSlotNum(i)]=(ItemStack)((Optional)this.getDataManager().get(symbolPar[i])).orNull();
