...
public abstract class EntityShell extends Entity
{
	...
	public static final DataParameter<Float> Xlower=EntityDataManager.<Float>createKey(EntityShell.class,DataSerializers.FLOAT);
	public static final DataParameter<Float> Ylower=EntityDataManager.<Float>createKey(EntityShell.class,DataSerializers.FLOAT);
	public static final DataParameter<Float> Zlower=EntityDataManager.<Float>createKey(EntityShell.class,DataSerializers.FLOAT);
	public static final DataParameter<Integer> Xupper=EntityDataManager.<Integer>createKey(EntityShell.class,DataSerializers.VARINT);
	public static final DataParameter<Integer> Yupper=EntityDataManager.<Integer>createKey(EntityShell.class,DataSerializers.VARINT);
	public static final DataParameter<Integer> Zupper=EntityDataManager.<Integer>createKey(EntityShell.class,DataSerializers.VARINT);

	public EntityShell(World par1World)
	{
		super(par1World);
		this.dataManager.register(this.Xlower, (float)this.posX-MathHelper.floor_double(this.posX));
		this.dataManager.register(this.Ylower, (float)this.posY-MathHelper.floor_double(this.posY));
		this.dataManager.register(this.Zlower, (float)this.posZ-MathHelper.floor_double(this.posZ));
		this.dataManager.register(this.Xupper, MathHelper.floor_double(this.posX));
		this.dataManager.register(this.Yupper, MathHelper.floor_double(this.posY));
		this.dataManager.register(this.Zupper, MathHelper.floor_double(this.posZ));
	}
	...
	public void onUpdate()
	{
		...
		if(worldObj.isRemote){
			this.posX=(double)this.dataManager.get(this.Xupper)+(double)this.dataManager.get(this.Xlower);
			this.posY=(double)this.dataManager.get(this.Yupper)+(double)this.dataManager.get(this.Ylower);
			this.posZ=(double)this.dataManager.get(this.Zupper)+(double)this.dataManager.get(this.Zlower);
			return;
		}
		...
		for(int i=1;i<=j;i++){
			...
			int minX = MathHelper.floor_double(this.getEntityBoundingBox().minX - 0.0D);
			int minY = MathHelper.floor_double(this.getEntityBoundingBox().minY - 0.0D);
			int minZ = MathHelper.floor_double(this.getEntityBoundingBox().minZ - 0.0D);
			int maxX = MathHelper.floor_double(this.getEntityBoundingBox().maxX + 0.0D);
			int maxY = MathHelper.floor_double(this.getEntityBoundingBox().maxY + 0.0D);
			int maxZ = MathHelper.floor_double(this.getEntityBoundingBox().maxZ + 0.0D);
			...
			if (this.worldObj.isAreaLoaded(new BlockPos(minX, minY, minZ), new BlockPos(maxX, maxY, maxZ)))
			{
				for (int x = minX; x <= maxX; ++x)
				{
					for (int y = minY; y <= maxY; ++y)
					{
						for (int z = minZ; z <= maxZ; ++z)
						{
							...
							if(!(this.worldObj.isAirBlock(targetPos))){
								
								Block target=this.worldObj.getBlockState(targetPos).getBlock();
								AxisAlignedBB tAABB=target.getBlockState().getBaseState().getCollisionBoundingBox(this.worldObj, targetPos);
								if(tAABB!=null){
									tAABB=tAABB.offset(targetPos);
									//System.out.println("shell:"+this.getEntityBoundingBox()+" block:"+tAABB);
									if(this.getEntityBoundingBox().intersectsWith(tAABB)){
										double relativeX,relativeY,relativeZ;
										relativeX=this.posX-(tAABB.maxX+tAABB.minX)/2;
										relativeY=this.getCentreYOffset()-(tAABB.maxY+tAABB.minY)/2;
										relativeZ=this.posZ-(tAABB.maxZ+tAABB.minZ)/2;
										...
										double hardness;
										if(target.getBlockState().getBaseState().getBlockHardness(worldObj,targetPos)==-1){
											hardness=Float.MAX_VALUE;
										}else{
											hardness=1.5*(target.getBlockState().getBaseState().getBlockHardness(worldObj,targetPos)+(2.0-target.getBlockState().getBaseState().getBlockHardness(worldObj,targetPos))/2)/this.destruction/ShellCore.destructionRate;
										}
											...
											if(Math.abs(relativeX)>=Math.abs(relativeY)&&Math.abs(relativeX)>=Math.abs(relativeZ)){
												...
												if(!((target instanceof BlockDynamicLiquid)||(target instanceof BlockStaticLiquid))&&!xflag){
													if(motionX>0&&relativeX<0){
														this.onCollidedWithBlock(targetPos,hardness, EnumFacing.WEST,tAABB);
														if(this.motionX>hardness){
															this.onBreakBlock(targetPos,hardness, EnumFacing.WEST,tAABB);
														}else{
															xflag=true;
															this.onFailedToBreakBlock(targetPos,hardness, EnumFacing.WEST,tAABB);
														}
													}else if(motionX<=0&&relativeX>=0){
														this.onCollidedWithBlock(targetPos,hardness, EnumFacing.EAST,tAABB);
														if(-this.motionX>hardness){
															
															this.onBreakBlock(targetPos,hardness, EnumFacing.EAST,tAABB);
														}else{
															xflag=true;
															
															this.onFailedToBreakBlock(targetPos,hardness, EnumFacing.EAST,tAABB);
														}
													}
												}
												...
												if(!((target instanceof BlockDynamicLiquid)||(target instanceof BlockStaticLiquid))&&!yflag){
													if(motionY>0&&relativeY<0){
														this.onCollidedWithBlock(targetPos,hardness, EnumFacing.DOWN,tAABB);
														if(this.motionY>hardness){
															
															this.onBreakBlock(targetPos,hardness, EnumFacing.DOWN,tAABB);
														}else{
															yflag=true;
															
															this.onFailedToBreakBlock(targetPos,hardness, EnumFacing.DOWN,tAABB);
														}
													}else if(motionY<=0&&relativeY>=0){
														this.onCollidedWithBlock(targetPos,hardness, EnumFacing.UP,tAABB);
														if(-this.motionY>hardness){
															
															this.onBreakBlock(targetPos,hardness, EnumFacing.UP,tAABB);
														}else{
															yflag=true;
															
															this.onFailedToBreakBlock(targetPos,hardness, EnumFacing.UP,tAABB);
														}
													}
												}
												...
												if(!((target instanceof BlockDynamicLiquid)||(target instanceof BlockStaticLiquid))&&!zflag){
													if(motionZ>0&&relativeZ<0){
														this.onCollidedWithBlock(targetPos,hardness, EnumFacing.NORTH,tAABB);
														if(this.motionZ>hardness){
															
															this.onBreakBlock(targetPos,hardness, EnumFacing.NORTH,tAABB);
														}else{
															zflag=true;
															
															this.onFailedToBreakBlock(targetPos,hardness, EnumFacing.NORTH,tAABB);
														}
													}else if(motionZ<=0&&relativeZ>=0){
														this.onCollidedWithBlock(targetPos,hardness, EnumFacing.SOUTH,tAABB);
														if(-this.motionZ>hardness){
															
															this.onBreakBlock(targetPos,hardness, EnumFacing.SOUTH,tAABB);
														}else{
															zflag=true;
															
															this.onFailedToBreakBlock(targetPos,hardness, EnumFacing.SOUTH,tAABB);
														}
													}
												}
											}
			...
			/*
        this.getEntityBoundingBox().offset(motionX/j, motionY/j, motionZ/j);
        ...
        this.posZ = (this.getEntityBoundingBox().minZ + this.getEntityBoundingBox().maxZ) / 2.0D;
			...
			List entityList=worldObj.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox());
		...
		this.dataManager.set(this.Xlower, (float)(this.posX-MathHelper.floor_double(this.posX)));
		this.dataManager.set(this.Ylower, (float)(this.posY-MathHelper.floor_double(this.posY)));
		this.dataManager.set(this.Zlower, (float)(this.posZ-MathHelper.floor_double(this.posZ)));
		this.dataManager.set(this.Xupper, MathHelper.floor_double(this.posX));
		this.dataManager.set(this.Yupper, MathHelper.floor_double(this.posY));
		this.dataManager.set(this.Zupper, MathHelper.floor_double(this.posZ));
	...
	public AxisAlignedBB getCollisionBoundingBox(){
		return this.getEntityBoundingBox().expand(-0.1,-0.1,-0.1);
	}
	 ...
	 * @param tAABB AxisAlignedBoundingBox of the block
	 */
	public void onFailedToBreakBlock(BlockPos pos,double hardness,EnumFacing fd,AxisAlignedBB tAABB){
		switch(fd){
		
		case WEST:
			this.getEntityBoundingBox().calculateXOffset(tAABB, 0);
		...
		case EAST:
			...
			break;
		case DOWN:
			this.getEntityBoundingBox().calculateYOffset(tAABB, 0);
		...
		case UP:
			...
			break;
		case NORTH:
			this.getEntityBoundingBox().calculateZOffset(tAABB, 0);
	...
	public void onCollidedWithEntity(Entity entity){
		...
		if (!this.getPassengers().contains(entity) && entity.canBePushed())
		{
