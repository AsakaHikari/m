...
public class ItemBuildingBoots extends ItemArmor
{
	...
	public void onArmorTick(World world, EntityPlayer entity, ItemStack stack)
	{
		...
		int x = MathHelper.floor(entity.posX+2*entity.motionX);
		int y = MathHelper.floor(entity.posY-0.95)-this.type;
		int z = MathHelper.floor(entity.posZ+2*entity.motionZ);
	...
	public static double getMaxY(World w,Entity e,int x,int y,int z){
		...
		id.addCollisionBoxToList(w.getBlockState(pos),w,pos, new AxisAlignedBB(x, y, z, x+1, y+1, z+1),list,e,false);
		if(list.isEmpty()){
			return y-1;
		}
