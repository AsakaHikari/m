	...
	public IMessage onMessage(MessageCannonGui message, MessageContext ctx) {
		...
		for(int i=0;i<=type;i++){
			...
			if ( (world.getBlockState(new BlockPos(x, y, z)).getBlock().isReplaceable(world, new BlockPos(x, y, z))) && solid>=2){
				...
				if(stackToUse!=null && 
						stackToUse.onItemUse(entity, entity.world, new BlockPos(x, y, z),EnumHand.MAIN_HAND,
					...
					if(stackToUse!=null && 
							stackToUse.onItemUse(entity, entity.world, new BlockPos(x, y, z),EnumHand.OFF_HAND,
									EnumFacing.DOWN , 0, 0, 0)==EnumActionResult.SUCCESS){
						if(stackToUse.getCount()<1){
