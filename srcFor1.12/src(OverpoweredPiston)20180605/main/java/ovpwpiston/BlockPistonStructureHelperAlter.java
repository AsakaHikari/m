...
public class BlockPistonStructureHelperAlter
{
    ...
    public BlockPistonStructureHelperAlter(World worldIn, BlockPos posIn, EnumFacing pistonFacing, boolean extending)
    {
        this.world = worldIn;
    ...
    private boolean addBlockLine(BlockPos origin, EnumFacing p_177251_2_)
    {
        ...
        {
        	int blocksMoveNum=12;
        	List<Entity> list1 = this.world.getEntitiesWithinAABBExcludingEntity((Entity)null,new AxisAlignedBB(pistonPos).expand(1.98,1.98,1.98).expand(-1.98,-1.98,-1.98));
        	
        	for(Entity e:list1){
        		if(e instanceof EntityEnderCrystal){
        			blocksMoveNum+=12;
        			//System.out.println(this.world.isRemote+","+e);
        		}
        	}
        	
            int i = 1;

            if (i + this.toMove.size() > blocksMoveNum)
            {
                return false;
            }
            else
            {
                ...
                while (true)
                {
                    ...
                    if (this.toMove.size() >= blocksMoveNum)
                    {
                        return false;
                    }
