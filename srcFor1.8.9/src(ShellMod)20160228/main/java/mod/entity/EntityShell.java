					...
					for (int y = minY; y <= maxY; ++y)
					{
						for (int z = minZ; z <= maxZ; ++z)
						{
							...
							if(!(this.worldObj.isAirBlock(targetPos))){
								...
								target.setBlockBoundsBasedOnState(worldObj, targetPos);
								AxisAlignedBB tAABB=target.getCollisionBoundingBox(worldObj,targetPos,this.worldObj.getBlockState(targetPos));
								if(tAABB!=null){
									if(this.getEntityBoundingBox().intersectsWith(tAABB)){
										...
										double hardness;
										if(target.getBlockHardness(worldObj, targetPos)==-1){
											hardness=Float.MAX_VALUE;
										}else{
											hardness=1.5*(target.getBlockHardness(worldObj,targetPos)+(2.0-target.getBlockHardness(worldObj,targetPos))/2)/this.destruction/ShellCore.destructionRate;
										}
